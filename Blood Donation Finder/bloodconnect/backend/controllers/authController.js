...
const register = async (req, res) => {
  try {
    const { first_name, last_name, email, password, phone, city, blood_group, role } = req.body;
    ...
    const [result] = await db.query(
      `INSERT INTO users (first_name, last_name, email, password, phone, city, blood_group, role)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [first_name, last_name, email, hashedPassword, phone || null, city, blood_group, role || 'donor']
    );
    ...
    return res.status(201).json({
      ...
      token,
      user: { id: result.insertId, first_name, last_name, email, role: role || 'donor', city, blood_group }
    });
